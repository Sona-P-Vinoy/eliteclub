      ...
      <div class="max-width">
        ...
        <ul class="menu">
          <li><a href="display_team.php" class="menu-btn">Home</a>
            <li><a href="view_course.php" class="menu-btn">View Course</a>
...
<div class="container" style="padding-top: 10%; font-weight: bold;">
          <center><h3>Course Details</h3></center>
             ...
             <?php
              $query1 = "select * from `register` where `reg_email`='$email'";
              $data1 = mysqli_query($con,$query1);
              while($res1=mysqli_fetch_assoc($data1)){
                $reg_id = $res1['reg_id'];
                //echo $reg_id;
              $query2 = "select * from `course_enroll` where `reg_id`='$reg_id'";
              $data2 = mysqli_query($con,$query2);
              while($res2=mysqli_fetch_assoc($data2)){
                $ce_id = $res2['ce_id'];
                $course_id = $res2['course_id'];
                
              $query3 = "select * from `training_programme` where `tp_id`='$course_id'";
              $data3 = mysqli_query($con,$query3);
              while($res3=mysqli_fetch_assoc($data3)){
                $tp_id = $res3['tp_id'];
                $tp_name = $res3['tp_name'];
                $tp_start_date = $res3['tp_start_date'];
                $tp_end_date = $res3['tp_end_date'];
                $tp_desc = $res3['tp_desc'];
