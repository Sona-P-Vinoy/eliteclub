...
$tid = $_POST['td_1'];
//echo $tid;
        $query = "UPDATE `team_reg` SET `team_status`= 1 WHERE `teamr_id` = '$tid'";
...
$status="Rejected";    
$tid = $_POST['td_1'];
$query = "UPDATE `team_reg` SET `team_status`= 2 WHERE `teamr_id` = '$tid'";
        echo '<script>alert("Status Updated!")</script>';
             if (mysqli_query($con,$query))
              {
              echo "Rejected";
       ...
       <div class="recent-sales box">
        ...
        <div class="container">
          ...
          <table id ="customers">
            ...
            $query = "SELECT `teamr_id`, `teamr_name`, `team_sec_name`, `team_sec_email`, `team_img`, `team_ground_addr`, `team_sec_phn`, `team_gen`, `team_desc`, `team_status` FROM `team_reg` WHERE `team_status` = 0";
            ...
            while($result = mysqli_fetch_assoc($row)){
              ...
              $query1 = "SELECT `gen_name` FROM `gen_type` WHERE `gen_id` = '$team_gen'";
              $row1 = mysqli_query($con,$query1);
              if($result1 = mysqli_fetch_assoc($row1)){
                $gename = $result1['gen_name'];
            ?>
            <form method="post" action="">
            <tr>
            <td><input type="hidden" name="td_1" value="<?php echo $team_id;?>"></td>
