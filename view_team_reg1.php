...
<body>
  ...
  <div class="container">
    ...
    <table id ="customers">
     ...
     $sql = "SELECT `reg_id`,`reg_status` from `register` WHERE `reg_email`='$email'";
      $res = mysqli_query($con);
      if($data = mysqli_fetch_assoc($res)){
        $r = $data['reg_id'];
     //echo $pname;
     $query = "SELECT `teamr_id`,`reg_team_id`, `teamr_name`, `team_sec_name`, `team_sec_email`, `team_img`, `team_ground_addr`, `team_sec_phn`, `team_gen`, `team_desc`, `team_status` FROM `team_reg` WHERE `reg_team_id` = '$r'";
